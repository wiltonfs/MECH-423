...
// P3 Switches
#define P3_SWITCHES (FINE_ADJST | MOD_2_ENBL | THERM_CR_0 | THERM_CR_1)
...
// P2 Switches
#define P2_SWITCHES (MOD_3_ENBL)
#define MOD_3_ENBL (BIT7)

// Handy references
#define MODULE_3_ENABLED ((P2IN & MOD_3_ENBL) > 0)
...
void SetupBinaryInputSwitches()
{
    ...
    P3SEL1 &= ~(P3_SWITCHES);
    P3SEL0 &= ~(P3_SWITCHES);
    // Configure switches as a digital input
    P2DIR  &= ~(P2_SWITCHES);
    P2SEL1 &= ~(P2_SWITCHES);
    P2SEL0 &= ~(P2_SWITCHES);
    ...
    P2OUT &= ~(P2_SWITCHES);  // Pulldown             (L) pg. 314
    P2REN |= (P2_SWITCHES);   // Enable resistors     (L) pg. 315
}
...
void ReadFromBinaryInputSwitches(volatile unsigned char* d1, volatile unsigned char* d2)
{
    ...
    *d1 |= ((P3IN & P3_SWITCHES) << 2);
    // D2: first bit is module 3 enable (bit 7)
    *d2 |= ((P2IN & MOD_3_ENBL) >> 7);
}
